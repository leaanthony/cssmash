...
// decodeRune replicates the lenient rune decoding of the lexer (parse.Input.PeekRune),
// which does not validate UTF-8. Identifier classification must be consistent between
// writing minified output and relexing it, even for invalid UTF-8 sequences.
func decodeRune(b []byte) (rune, int) {
	c := b[0]
	if c < 0xC0 || len(b) < 2 {
		return rune(c), 1
	} else if c < 0xE0 || len(b) < 3 {
		return rune(c&0x1F)<<6 | rune(b[1]&0x3F), 2
	} else if c < 0xF0 || len(b) < 4 {
...
func decodeLastRune(b []byte) rune {
	start := len(b) - 1
	for i := len(b) - 1; 0 <= i && len(b)-i <= 4; i-- {
		if b[i] < 0x80 || 0xC0 <= b[i] {
			start = i
			break
		}
	}
	if b[start] < 0xC0 {
		...
		return utf8.RuneError // orphaned continuation byte
	}
	r, _ := decodeRune(b[start:])
	return r
}
