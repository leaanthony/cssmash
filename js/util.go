...
var binaryLeftPrecMap = map[js.TokenType]js.OpPrec{
	...
	// logical assignment; absent from these maps the lookup yields the zero
	// OpPrec, which reads as the loosest binding there is and makes every
	// precedence decision about &&= ||= ??= wrong
...
// isEmptyStringLiteral returns whether a string literal (quotes included in
// data) evaluates to the empty string. Line continuations (a backslash
// followed by a line terminator) contribute no characters; every other escape
// sequence and every literal character contributes at least one.
func isEmptyStringLiteral(data []byte) bool {
	s := data[1 : len(data)-1]
	for i := 0; i < len(s); {
		if s[i] != '\\' || i+1 == len(s) {
			return false
...
func isFalsy(i js.IExpr) (bool, bool) {
	...
	if lit, ok := i.(*js.LiteralExpr); ok {
		tt := lit.TokenType
		d := lit.Data
		if tt == js.FalseToken || tt == js.NullToken || tt == js.StringToken && isEmptyStringLiteral(lit.Data) {
...
func (m *jsMinifier) optimizeCondExpr(expr *js.CondExpr, prec js.OpPrec) js.IExpr {
	...
	if truthy, ok := isTruthy(expr.Cond); truthy && ok {
		...
		if trueX && falseY || falseX && trueY {
		...
		} else if (trueX || trueY) && condFoldsSmaller(expr.Cond) {
			// trueX != trueY
		...
		} else if (falseX || falseY) && condFoldsSmaller(expr.Cond) {
			// falseX != falseY
			...
			binaryLeftPrecMap[js.AndToken] <= exprPrec(expr.Cond) && binaryRightPrecMap[js.AndToken] <= exprPrec(condExpr.Cond) {
			// Nested conditional expression with the same false body. Merging
			// drops one copy of that body and costs one "&&", so it pays only
			// while neither condition has to be parenthesised: 0??0?0?0:C:C
			// is shorter than (0??0)&&0?0:C.
...
// needsExtraDot reports whether a '.' written directly after a numeric literal
// would be absorbed as its decimal point instead of starting a member access.
// Only a bare run of decimal digits can still take one: a literal that already
// has a decimal point, an exponent, or a radix prefix cannot. So 5 has to be
// written 5..x, while 5.5, 1e6 and 0x10 take a single dot.
func needsExtraDot(data []byte) bool {
	if len(data) == 0 {
		return false
	}
	for _, c := range data {
		// Numeric separators are part of the digit run: 1_0 is still an
		// integer literal that can take a decimal point.
		if (c < '0' || '9' < c) && c != '_' {
			return false
		}
	}
...
// condFoldsSmaller reports whether rewriting a conditional with a boolean
// literal branch into ||/&& is actually worth it. The rewrite has to coerce
// the condition with !!, and when the condition also needs parentheses at
// unary precedence that costs more than the ?: it replaces: 0%0?!0:0 is eight
// bytes, !!(0%0)||0 is ten. A condition that is already boolean needs neither.
//
// The surviving branch can also need parentheses, which this deliberately does
// not account for: refusing those cases costs more bytes across the corpus
// than it saves, because the fold enables later rewrites that more than pay
// for the parentheses. The consequence is that a second minification pass can
// occasionally regroup into a marginally longer form; see FuzzMinify.
func condFoldsSmaller(cond js.IExpr) bool {
	return isBooleanExpr(cond) || js.OpUnary <= exprPrec(cond)
}
