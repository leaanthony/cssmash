...
// mergeableVarDeclTarget resolves an assignment target through its Link chain
// and returns the canonical variable if the assignment may be merged into decl
// as a definition: it must be declared with `var` in the same function scope as
// decl. An assignment to an enclosing function's variable must stay a plain
// assignment; merging it would re-declare the variable locally and shadow the
// outer binding.
func mergeableVarDeclTarget(decl *js.VarDecl, iexpr js.IExpr) *js.Var {
	v, ok := iexpr.(*js.Var)
	if !ok {
		return nil
	}
	...
	for _, d := range decl.Scope.Func.Declared {
		if d == v {
			return v
		}
	}
	...
	if decl2, ok := exprStmt.Value.(*js.VarDecl); ok {
		...
		for i := 0; i < len(commaExpr.List); i++ {
			...
			if src, ok := item.(*js.VarDecl); ok {
				...
				if v := mergeableVarDeclTarget(decl, binaryExpr.X); v != nil {
	...
	if decl.TokenType == js.VarToken {
		...
		slices.SortStableFunc(decl.List[start:], func(a, b js.BindingElement) int {
			if a.Default == nil && b.Default == nil {
				// Sort single-length variable names. Restricted to ASCII:
				// identOrder is an ASCII table, and a lone non-ASCII byte is
				// the truncated head of a UTF-8 sequence, which must not be
				// moved away from the end of the input where it would take
				// the bytes that follow it as continuations.
				if va, ok := a.Binding.(*js.Var); ok && len(va.Data) == 1 && va.Data[0] < utf8.RuneSelf {
					if vb, ok := b.Binding.(*js.Var); ok && len(vb.Data) == 1 && vb.Data[0] < utf8.RuneSelf {
