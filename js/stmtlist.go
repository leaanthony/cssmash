...
func optimizeStmtList(list []js.IStmt, blockType blockType) []js.IStmt {
	...
	// if(a)return; rest  =>  if(!a)rest
	//
	// A bare return does exactly what falling off the end of a function body
	// already does, and a bare continue likewise for a loop body. So the early
	// exit can be inverted and the statement it was guarding against pulled
	// into it, which the writer then usually collapses further to a||rest.
	//
	// Runs after the merging above, so a tail of expression statements has
	// already become the single statement this needs. Folding shortens the
	// tail by one, which can expose another such pair, hence the loop.
	if blockType == functionBlock || blockType == iterationBlock {
		for 1 < j {
			ifStmt, ok := list[j-2].(*js.IfStmt)
			if !ok || !isEmptyStmt(ifStmt.Else) || !isRedundantExit(ifStmt.Body, blockType) || !canBeIfBody(list[j-1]) {
				break
			}
			ifStmt.Body = list[j-1]
			ifStmt.Cond = invertCond(ifStmt.Cond)
			list[j-2] = optimizeStmt(ifStmt)
			j--
		}
	}
	return list[:j]
...
// isRedundantExit reports whether stmt transfers control to exactly where
// running off the end of the enclosing block would have gone anyway.
func isRedundantExit(stmt js.IStmt, blockType blockType) bool {
	switch blockType {
	case functionBlock:
		returnStmt, ok := stmt.(*js.ReturnStmt)
		return ok && (returnStmt.Value == nil || isUndefined(returnStmt.Value))
	case iterationBlock:
		branchStmt, ok := stmt.(*js.BranchStmt)
		return ok && branchStmt.Type == js.ContinueToken && branchStmt.Label == nil
	}
	return false
}

// canBeIfBody reports whether stmt may legally and equivalently become the
// body of an if statement. A lexical declaration may not: `if(a)let x=1` is a
// syntax error, and a function or class declaration would move into a narrower
// scope than the statement list it came from.
func canBeIfBody(stmt js.IStmt) bool {
	switch decl := stmt.(type) {
	case *js.VarDecl:
		return decl.TokenType == js.VarToken
	case *js.FuncDecl, *js.ClassDecl:
		return false
	}
	return true
}

// invertCond negates an expression used as a condition. Since the result is
// only ever consumed as a boolean, a leading ! can be dropped rather than
// doubled.
func invertCond(cond js.IExpr) js.IExpr {
	for {
		group, ok := cond.(*js.GroupExpr)
		if !ok {
			break
		}
		cond = group.X
	}
	if unary, ok := cond.(*js.UnaryExpr); ok && unary.Op == js.NotToken {
		return unary.X
	}
	return &js.UnaryExpr{js.NotToken, groupExpr(cond, js.OpUnary)}
}
