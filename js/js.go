...
// Minify minifies JS data, it reads from r and writes to w.
func (o *Minifier) Minify(_ *cssmash.M, w io.Writer, r io.Reader, params map[string]string) error {
	z := parse.NewInput(r)
	defer z.Restore()
	...
	case *js.ExportStmt:
		...
		if stmt.Decl != nil {
			...
			if len(stmt.List) == 1 && (len(stmt.List[0].Name) == 1 && stmt.List[0].Name[0] == '*' || stmt.List[0].Name == nil && len(stmt.List[0].Binding) == 1 && stmt.List[0].Binding[0] == '*') {
				...
				// An empty clause still has to be written: `export{}` is what
				// marks a file as a module, and a bare `export` is a syntax
				// error.
				m.write(openBraceBytes)
				for i, item := range stmt.List {
					if i != 0 {
...
func (m *jsMinifier) minifyExpr(i js.IExpr, prec js.OpPrec) {
	...
	case *js.BinaryExpr:
		...
		if folded, ok := foldTypeofUndefined(expr); ok {
			// typeof x=="undefined" => typeof x>"u"; see typeof.go. Applied
			// before the operand precedences below are read, so they describe
		...
		//
		// Never for an assignment, where X is the target rather than an
		// operand: (a,b)=c would become a,b=c, which regroups as a,(b=c).
		if prec <= js.OpExpr && binaryLeftPrecMap[expr.Op] != js.OpLHS {
		...
		if expr.Op == js.PostIncrToken || expr.Op == js.PostDecrToken {
			...
			if expr.Op == js.DeleteToken || expr.Op == js.VoidToken || expr.Op == js.TypeofToken || expr.Op == js.AwaitToken {
				...
				if lit, ok := expr.X.(*js.LiteralExpr); ok && lit.TokenType == js.StringToken {
					...
					if num := cssmash.Number(lit.Data, m.o.Precision); len(num) == 1 && num[0] == '0' {
						m.write(zeroBytes)
					} else {
						m.write(oneBytes)
					}
		...
		if group, ok := expr.X.(*js.GroupExpr); ok {
			if lit, ok := group.X.(*js.LiteralExpr); ok && (lit.TokenType == js.DecimalToken || lit.TokenType == js.IntegerToken) {
				...
				if lit.TokenType == js.DecimalToken {
					// decimalNumber, not cssmash.Number: the latter is the
					// CSS normalizer and leaves JS numeric separators in
					// place, so .0_0 came out as the invalid .0_
					data = decimalNumber(lit.Data, m.o.Precision)
				}
				m.write(data)
				if needsExtraDot(data) {
					// 5.toString would read the dot as a decimal point
					m.write(dotBytes)
				}
				m.write(dotBytes)
				m.write(yData)
	...
	case *js.IndexExpr:
		...
		if lit, ok := expr.Y.(*js.LiteralExpr); ok && lit.TokenType == js.StringToken && 2 < len(lit.Data) {
			if isIdent := js.AsIdentifierName(lit.Data[1 : len(lit.Data)-1]); isIdent {
				...
				m.write(openBracketBytes)
				m.write(cssmash.Number(lit.Data[1:len(lit.Data)-1], 0))
				m.write(closeBracketBytes)
				break
			}
		}
